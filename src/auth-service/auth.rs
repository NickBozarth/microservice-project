...
impl Auth for AuthService {
    async fn sign_in(
        ...
        let result: Option<String> = self.users_service
            .lock()
            .expect("users_service lock is poisoned")
        ...
        let session_token: String = self.sessions_service
            .lock()
            .expect("sessions_service lock is poisoned")
    ...
    async fn sign_up(
        ...
        match result {
            Ok(_) =>  Ok(Response::new(SignUpResponse { status_code: StatusCode::Success.into() })),
            Err(_) => Ok(Response::new(SignUpResponse { status_code: StatusCode::Failure.into() }))
        }
    }
